...
Own Layer 2b (K-NN retrieval module), Layer 3 (Pareto optimizer), Layer 4 (SDXL visual compiler), and Layer 5 (LLM interpreter). The LLM's role is strictly a communicative interpreter — it translates structured diagnostic data (including K-NN neighbour data) into human-readable explanations, never deriving scientific predictions itself.
...
- **System prompt:** Write the scientific communicator prompt in `backend/app/agent/prompts.py`. The LLM receives pre-computed metrics (CTE, Δ_stress, crazing_risk_tier, GNN surface/transparency/color class, K-NN nearest neighbour recipes with community notes, Stull zone) and explains them to the user. It does NOT calculate chemistry or predict properties.
...
- **Objective function:** Implement multi-objective search in `backend/app/optimizer/pareto.py`:
  - Minimize Δ_stress = |Glaze_CTE - Clay_CTE|
  - Maximize target surface class probability (from GNN)
- **Search strategy:** Constrained Grid Search over the top 3 primary variable inputs. Unconstrained evolutionary search is avoided for latency reasons. Constraints: total = 100%, each ingredient ∈ [0, 100].
...
- **Dynamic prompt builder:** Write `backend/app/render/sdxl.py` that constructs descriptive prompts from GNN surface/transparency predictions and crazing risk tier:
  - Surface = glossy → "high-gloss reflective silicate glass matrix"
  - Surface = stony matte → "rough micro-crystalline ceramic surface"
  - Crazing risk tier = "High" or "Extreme" → "hairline crack network across glaze surface"
...
- **Integration test:** Full pipeline: recipe → UMF → CTE → GNN → K-NN neighbours → Optimizer → SDXL → LLM → response.

## Deliverables

- `backend/app/retrieval/knn.py` — Faiss/USearch K-NN vector retrieval with GlazyBench corpus index
- `backend/app/agent/core.py` — LLM interpreter with structured JSON output (includes neighbour data)
...
- `backend/app/optimizer/pareto.py` — Constrained grid search for multi-objective formulation search
